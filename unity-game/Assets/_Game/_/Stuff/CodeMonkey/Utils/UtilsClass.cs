...
namespace CodeMonkey.Utils {
    ...
    public static class UtilsClass {
        ...
        // Modify UtilsClass.GetMouseWorldPosition() to handle touches
        public static Vector3 GetTouchWorldPosition(Vector2 touchPosition)
        {
            Vector3 vec = GetMouseWorldPositionWithZ(touchPosition, Camera.main);
            vec.z = 0f;
            return vec;
        }
        ...
        public static bool IsTouchOnUI(int fingerId, Collider2D collider, Camera cam)
        {
            // Check if the touch is on the UI joystick's BoxCollider2D
            if (collider != null)
            {
                Vector2 touchPosition = Input.GetTouch(fingerId).position;
                Vector3 worldTouchPosition = cam.ScreenToWorldPoint(touchPosition);

                if (collider.OverlapPoint(worldTouchPosition))
                {
                    return true; // Touch is on the UI joystick
                }
            }

            return false; // Touch is not on the UI joystick
        }
